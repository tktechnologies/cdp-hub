...
cdp_skus (Google Sheets)
  → cdp_router (inline StokAPI dispatch via router_stokapi.js)
...
| 5 | Secrets aligned | Key Vault `cdp-scrapers-kv-prod` → `cdp-n8n-prod` + `cdp-muv-api` | API key + callback secret match across n8n/API |
| 6 | Live workflow sync | From monorepo root: `python3 scripts/sync_workflow_code_from_shared.py` then `make sync-n8n` | Repo JSON matches n8n after changes (user approval for publish) |
| 7 | Redis stream | `XLEN muvstok:jobs`, `XPENDING` | No huge pending backlog before audit |
| 8 | Dead letter | `XLEN muvstok:jobs:dead-letter` | Review historical DLQ; requeue or purge per ops |
...
3. Trigger dispatch via `cdp_router` (Telegram `.sku` or manual router run).
...
| **JSON vs SDK drift** | Medium | Repo JSON missing `CDP_MUVSTOK_MAX_SKUS` / `AUDIT_SAMPLE` in DQ node; SDK has MAX_SKUS | Run `make sync-n8n` from monorepo root after aligning JSON/SDK |
...
| **Full sheet without cap** | Critical | Accidental run on 1k rows | Use `CDP_MUVSTOK_AUDIT_SAMPLE=5` or `CDP_MUVSTOK_MAX_SKUS` on router dispatch |
...
6. Rollback: pause router triggers; disable worker scale-up if error rate spikes.
